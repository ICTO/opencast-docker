...
opencast_opencast_configure() {
  ...
  opencast_helper_replaceinfile "${OPENCAST_CONFIG}/etc/org.opencastproject.organization-mh_default_org.cfg" \
    "PROP_ORG_OPENCASTPROJECT_FILE_REPO_URL" \
    "PROP_ORG_OPENCASTPROJECT_ADMIN_UI_URL" \
    "PROP_ORG_OPENCASTPROJECT_ENGAGE_UI_URL"

  if opencast_helper_dist_migration ; then
    opencast_helper_replaceinfile "${OPENCAST_CONFIG}/etc/custom.properties" "ORG_OPENCASTPROJECT_MIGRATION_ORGANIZATION"
  fi
}
